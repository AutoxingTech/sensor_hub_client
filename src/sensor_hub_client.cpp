      ...
      m_scanMatchParser({0xba, 0xe4}), m_asyncSpinner(1, &m_callbackQueue), m_asyncHandle("~")
{
    ...
    ROS_INFO("SensorHubClient::SensorHubClient()");
...
int SensorHubClient::run()
{
    ...
    m_nh.param<std::string>("host_ip", m_hostIP, "172.17.0.2");
    ...
    while (ros::ok())
    {
        ...
        {
            ROS_INFO("TCP client is disconnected, will reconnect...");
            m_comStream->close();
        ...
        {
            ROS_INFO_THROTTLE(3, "m_comStream isConnected");
        }
